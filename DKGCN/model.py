...
class KGCN(torch.nn.Module):
    ...
    def forward(self, u, v,mask):
        ...
        item_embeddings = self._aggregate(user_embeddings, entities, relations,mask)
    ...
    def get_entity_mask(self,entities,mask):
        entities_mask = []
        for i in range(len(entities)):
            temp = []
            for entity in entities[i]:
                temp.append(mask[entity])
            temp = torch.stack(temp,0).unsqueeze(-1)
            entities_mask.append(temp)
        # entities_mask = torch.stack(entities_mask)
        return entities_mask

    def _aggregate(self, user_embeddings, entities, relations,mask):
        '''
        Make item embeddings by aggregating neighbor vectors
        '''
        # entity_vectors[hop,[batch_size,nebor,dim]]
        # 为的entity_vectors乘上mask
        entity_vectors = [self.ent(entity) for entity in entities]
        entities_mask = self.get_entity_mask(entities,mask)
        for i in range(len(entity_vectors)):
            entity_vectors[i] = entity_vectors[i]*entities_mask[i]
